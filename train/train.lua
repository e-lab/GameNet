...
function train:__init(opt)
   ...
   print(cp.r .. "Loading training data from " .. opt.datapath .. " ..." .. cp.reset)
   self.dataset = torch.load(datapath):float()
   self.dataset = self.dataset/self.dataset:max()
   print(cp.g .. "Loaded " .. self.dataset:size(1) .. " training image sequences" .. cp.reset)
   ...
   local model = require 'models.prednet'
   ...
   if self.dev == 'cuda' then
      ...
      self.model = nn.DataParallelTable(1, true, false):add(self.model:cuda(), gpuList)
...
function train:updateModel()
   ...
   -- height = c and height/2 or height
