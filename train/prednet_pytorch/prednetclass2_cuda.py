...
class MyDataset(Dataset):
    ...
    def getbatch(self):
        rand = np.random.randint(1,self.max-self.length-1,size = (self.batch))
...
class Prednet(nn.Module):
    ...
    def forward(self ,input, error, state,):
        ...
        for l in range(0, self.number_of_layers):
            ...
            if l == 0:
            	save_image(input_projection.data,'input.png')
            	save_image(state_projection.data,'preview.png')
    ...
    def initParams(self):
        ...
        try: 
            ...
            'linear_weight':     torch.zeros(512,2048).cuda(),
    ...
    def train(self,):
        target_sequence = Variable(torch.zeros(net.T, net.batch_size, net.output_channels, net.width_ratio * 2 ** 7, net.height_ratio * 2 ** 7).cuda())
        ...
        for epoch in range(0, self.max_epoch):
            ...
            loss = 0
            error = [Variable(torch.zeros(self.batch_size,2*self.Ahat_size_list[l],self.width_ratio*2 **(self.exp-l),self.height_ratio*2 **(self.exp-l)).cuda())
                for l in range(0,self.number_of_layers)]
            state = [(
                Variable(torch.zeros(self.batch_size, self.R_size_list[l], self.width_ratio*2 **(self.exp-l),self.height_ratio*2 **(self.exp-l)).cuda()),
                Variable(torch.zeros(self.batch_size, self.R_size_list[l], self.width_ratio*2 **(self.exp-l),self.height_ratio*2 **(self.exp-l)).cuda())
                ) for l in range(0,self.number_of_layers)]
